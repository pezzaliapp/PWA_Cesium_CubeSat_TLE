 ...
 * MIT 2025
 * Add-on ultra-safe: Subsolar point + Terminator con RAF privato
 */
'use strict';
...
/* =========================
   ADD-ON ULTRA-SAFE (RAF)
   =========================
   - Niente Cesium.Simon1994*, niente onTick.
   - Se qualcosa va storto, il blocco ignora l’errore e NON tocca la simulazione.
*/
(function(){
  let sunPoint = null;
  let terminator = null;
  ...
  // Sole approssimato in ECEF (formule autonome, niente API interne Cesium)
  function sunVecECEF(jd){
    try{
      ...
      // Versore Sole in ECI
      const x_eci = Math.cos(lambda);
      const y_eci = Math.cos(eps)*Math.sin(lambda);
      const z_eci = Math.sin(eps)*Math.sin(lambda);

      // ECI -> ECEF (uso matrice ICRF->fixed se disponibile; altrimenti lascio ECI)
      const icrfToFixed = Cesium.Transforms.computeIcrfToFixedMatrix(jd);
      if (icrfToFixed){
        const v = new Cesium.Cartesian3(x_eci, y_eci, z_eci);
        return Cesium.Matrix3.multiplyByVector(icrfToFixed, v, new Cesium.Cartesian3());
      }
      return new Cesium.Cartesian3(x_eci, y_eci, z_eci);
    }catch(_){
      return null;
    }
  }
  ...
  function updateOnce(){
    try{
      ensureEntities();
      if (!sunPoint || !terminator) return;

      const now = viewer.clock.currentTime;
      const sun = sunVecECEF(now);
      if (!sun) return;

      // Subsolare
      const dir = Cesium.Cartesian3.normalize(sun, new Cesium.Cartesian3());
      ...
      // Terminatore = grande cerchio ortogonale
      const pts = [];
      const N = 240;
  ...
  function loop(){
    try{ updateOnce(); }catch(_){}
    requestAnimationFrame(loop);
  }

  // Avvio una sola volta al load: non interagisce con Simula/Play/Reset
  if (!started){
    started = true;
    requestAnimationFrame(loop);
  }
